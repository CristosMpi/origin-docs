...
- Rosetta mounting features;
- internal clearances;
- radar mounting-hole positions;
- sensor openings;
- connector openings;
- BIT interface position;
- solar-support interfaces;
- base/mount geometry;
- mating surfaces.
...
- standoffs support the PCB correctly;
- no component contacts the enclosure unintentionally;
- connectors remain accessible;
- cables can be attached and removed;
- PCB removal is possible without destructive disassembly;
- fasteners do not bend the board.

Record any interference before modifying the part.

## Radar mounting test

Each C4001 radar mount should be tested for:
...
- no PCB bending;
- connector access;
- clearance from fasteners and walls;
- unobstructed sensing face.
...
- opening is centered on the sensing face;
- edges do not obstruct the intended field;
- mounting tolerance does not shift the sensor behind a wall;
- opening remains structurally sound;
- drainage/water-entry implications are understood.
...
- reach the BIT interface;
- attach/remove a BIT without removing ORIGIN from the site;
- identify orientation;
- avoid damaging adjacent cables;
- close/protect the interface after use where applicable.
...
Where threaded inserts or captive hardware are used, inspect for:
...
Mechanical cable validation should verify:

- no sharp bends beyond acceptable limits;
- no cable trapped between enclosure halves;
- strain is not transferred directly to PCB connectors;
- service loops do not enter sensing regions;
- cables do not interfere with fasteners;
- solar wiring can pass through the hollow support as intended.
...
- loosened internal parts;
- cracked mounts;
- connector movement;
- panel/support damage;
- rattling;
...
- rotation;
- loosening;
- soil/base interaction where applicable;
- access to service points;
- sensor orientation after installation.
...
Inspect loaded supports and interfaces after extended periods where possible, particularly:
...
Mechanical tests may use criteria such as:

- all intended components fit without forced deformation;
- sensors retain defined orientation;
- connectors remain accessible;
- no cable is pinched;
- enclosure closes consistently;
- no cracking after defined assembly cycles;
- supports remain aligned after defined load;
- unit remains stable on intended mount.

Exact numerical thresholds should be defined per released design rather than invented globally.

## Regression triggers

Repeat relevant tests after changes to:
...
- dimensional table;
- CAD screenshots;
- photographs before/after assembly;
- load-test setup photos;
- video of access/service test;
- damage photos;
- revision identifier.
...
- [Enclosure](../mechanical-design/enclosure.md)
- [Sensor Mounting](../mechanical-design/sensor-mounting.md)
- [Solar System](../mechanical-design/solar-system.md)
