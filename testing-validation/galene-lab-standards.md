...
- successful completion of a sequence.
...
The evidence should preserve the underlying information, such as:

- serial log;
- timestamped event file;
- video;
- test table;
- photograph of wiring;
...
For example, when testing a radar opening:

- keep sensor position fixed;
- keep firmware fixed;
- keep target route fixed;
- change only the opening geometry.
...
- operator effects;
- inconsistent mechanical positioning.
...
Public documentation should label values as either:

- manufacturer specification;
- design target;
- observed ORIGIN result;
- validated ORIGIN result.

## 10. Test failures explicitly

A test program that only checks expected success paths is incomplete.

Where safe and practical, deliberately create faults such as:

- unplugging a sensor;
- interrupting communications;
- rebooting the controller;
- disconnecting external power;
- presenting invalid input;
- moving or obstructing a sensor;
- removing a module;
...
- regulated bench supply for controlled power tests;
- known reference distance for radar tests;
- scale or force method for mechanical loading;
- thermometer/hygrometer for environmental context.
...
It can reveal details that are otherwise missed in written notes:
...
- one delayed startup among ten repetitions;
- temporary communication warning;
- unexpected temperature rise;
- visible enclosure flex;
- one false detection outside the acceptance window.
...
- firmware sensor-driver changes;
- power-management changes;
- enclosure geometry changes;
- radar opening changes;
- sensor angle changes;
- mounting-height changes;
- communications changes;
- configuration changes;
- AI or event-processing changes.
...
Before publishing a value as validated ORIGIN performance, confirm:

- the test configuration is known;
- the procedure is documented;
- the measurement is reproducible;
- the limitation is stated;
- the result applies to the current revision or is clearly revision-specific.
...
- do not modify historic fabric without permission;
- use reversible test fixtures where possible;
- protect visitors and staff from trip hazards;
- do not intentionally create unsafe electrical conditions;
- coordinate disruptive testing with site operators;
- follow local rules for drones and restricted areas.
