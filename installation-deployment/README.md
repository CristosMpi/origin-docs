...
At minimum, confirm that the correct hardware revision is identified; the current firmware version is known; Rosetta powers correctly; the expected sensors are detected; local logging works; configuration can be read back; operating notes are available; mounting hardware is complete; required tools and fasteners are packed; the deployment configuration has been prepared; and a rollback plan exists if commissioning fails.
