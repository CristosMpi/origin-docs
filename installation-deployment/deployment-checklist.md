...
Select one outcome **Accepted** — all required functions pass, **Accepted with limitations** — usable with documented non-critical limitations, **Rework required** — issues must be corrected before operation, and **Rejected / rollback** — deployment should not enter normal operation.
