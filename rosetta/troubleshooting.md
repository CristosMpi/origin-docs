...
Remove power and inspect for:

- solder bridges;
- reversed ICs;
- incorrect regulator/charger assembly;
- shorted capacitors;
- damaged components;
- incorrect footprint-to-part mapping.
...
- exact fitted regulator part;
- inductor values and footprints;
- input voltage;
- surrounding capacitors;
- enable/control pins;
- solder quality;
- PCB layout around the switching loop.
...
- correct programming connector/pinout;
- ground connection;
- logic-voltage level;
- reset/enable state;
- boot strap/button state;
- USB-UART or programmer operation;
- firmware board target;
- processor orientation and soldering.

## ESP32 resets repeatedly

Possible causes include:

- unstable regulated rail;
- insufficient current capability under load;
- brownout during radio transmission;
- watchdog restart;
- firmware crash;
- boot-strap pin conflict;
- external peripheral pulling a critical line.
...
- LIS3DH power rail;
- bus pull-ups where required;
- address selection;
- CS/address pin state;
- SCL/SDA or SPI continuity;
- firmware bus configuration;
...
- card format;
- firmware filesystem handling;
- power integrity during writes.

Test missing-card and card-insertion states separately.

## External sensor fails only when connected

Measure the header before connecting the sensor:
...
Stop assembly and compare:

- manufacturer package drawing;
- schematic symbol pin numbers;
- PCB footprint pad numbers;
- physical part orientation.
...
- firmware commit;
- test setup;
- symptoms before and after each change.
