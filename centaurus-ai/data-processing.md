...
A structured observation should generally include:

- message or observation ID;
- device identity;
- sensor identity;
- timestamp;
- observation type;
...
Source identity allows operators and developers to answer:

- which ORIGIN unit generated this data?
- which physical sensor produced it?
- was that source healthy at the time?
- what firmware version produced the record?
...
The processing path should distinguish between:
...
Where ordering matters, the system can use:
...
- converting device-specific state codes into shared quality states;
- enforcing consistent numerical representation;
- mapping observation types into a stable taxonomy.
...
Centaurus can associate a record with:
...
Depending on the deployment, data may be buffered:

- on the ORIGIN device;
- on a local gateway;
- in an ingestion queue;
- in persistent backend storage.
...
The architecture should define bounded behavior such as:

- queue limits;
- prioritization;
- controlled dropping of low-value telemetry where acceptable;
...
Retention can differ between:

- raw high-rate telemetry;
...
- labelling quality;
- separation between training and evaluation sets;
- representation of realistic deployment conditions;
- versioning;
- protection against accidental inclusion of sensitive information.
...
Different roles may need different views, for example:

- maintenance staff need device health;
- operators need current events;
- developers may need diagnostic traces;
...
If data is exported for research, testing, or reporting, the export should identify:

- date range;
- site/device scope;
- schema version;
- processing state;
- whether records are raw, normalized, or derived;
- any anonymization or filtering applied.

## Reproducibility

A historical Centaurus result should be reproducible as far as practical from:

- source observations;
- analysis version;
- model/ruleset version;
- configuration version;
- feature-processing version.
