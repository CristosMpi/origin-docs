...
- documented before and after service;
- performed with power safely isolated;
- followed by the required calibration or recommissioning;
- treated as a configuration/revision change when the replacement is not identical.
...
- calibration values relevant to the component;
...
- unit identity/provisioning information;
- connected modules;
...
- confirm no cable is trapped or strained;
- load the approved firmware;
- restore only compatible configuration;
- confirm unit identity;
- verify every expected sensor and module;
- run power, storage and communication checks;
- perform commissioning checks before returning to service.
...
- verify correct exposure to ambient conditions;
- confirm any shield or membrane is restored;
- verify units and scaling;
- compare readings with a suitable reference where available;
- check for plausible stabilization behavior.
...
- confirm polarity;
- secure the battery mechanically;
- prevent cable strain;
- verify charging behavior;
- verify power-management state;
- monitor for abnormal heat during initial operation.

Never return a swollen, punctured, leaking or overheated battery to service.

## Solar-panel replacement

When replacing the panel or supports:

- confirm panel electrical compatibility;
- restore the commissioned mechanical orientation;
- check all supports;
- verify the hollow third support cable path;
- prevent cable abrasion;
- inspect the enclosure cable entry;
- verify charging behavior after reassembly.
...
Replacement cables should match:
...
Route the replacement so it does not:

- obstruct sensors;
- create a water path;
- contact sharp edges;
- carry structural load;
- interfere with enclosure closure.

## Enclosure-part replacement

Replacing a shell, hatch, sensor face or mounting element can change:
...
Before fitting the new seal:

- clean the mating surfaces;
- remove debris;
- inspect the groove or compression surface;
- verify the seal is not twisted;
- ensure wiring does not cross the seal path.
...
- verify mechanical engagement;
- verify power and data interface;
- confirm software detection;
- run module-specific functional checks;
- ensure the Core remains healthy with the module attached.

## Storage replacement

Before storage replacement:

- attempt to preserve recoverable data;
- record corruption or failure evidence;
- confirm the approved storage format and capacity requirements;
- restore only known-good data/configuration.
...
Failed components should be labeled with:
...
- communication;
- no new warnings;
- no visible environmental-protection defect.
...
Escalate to development when the new part differs materially in:
